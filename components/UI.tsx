...
const UI: React.FC = () => {
  ...
  useEffect(() => {
    if (!latestBlock || cameraMode !== CameraMode.LIVE) return;
    if (selectedObject?.type === "tx") return;
    if (
      selectedObject?.type === "block" &&
      selectedObject.data.height === latestBlock.height
    ) {
      return;
    }
    selectObject({ type: "block", data: latestBlock });
  }, [cameraMode, latestBlock, selectObject, selectedObject]);

  const panelObject =
    selectedObject ??
    (latestBlock ? { type: "block" as const, data: latestBlock } : null);

  const toggleMode = () => {
    setCameraMode(
      cameraMode === CameraMode.LIVE ? CameraMode.FREE : CameraMode.LIVE
    );
  };
      ...
      <div className="pointer-events-auto absolute top-1/2 right-6 transform -translate-y-1/2 w-80 bg-[#1a1a20]/90 backdrop-blur-md border border-[#333] rounded-lg p-4 shadow-2xl text-white">
        <div className="flex justify-between items-center mb-4 border-b border-gray-700 pb-2">
          <h2 className="text-lg font-semibold flex items-center gap-2">
            ...
            {panelObject?.type === "tx" ? "Transaction" : "Block Details"}
          </h2>
          {selectedObject && (
            <button
        ...
        <div className="space-y-3 text-sm min-h-[220px] flex flex-col justify-between">
          {panelObject ? (
            panelObject.type === "block" ? (
              <>
                <div>
                  <span className="text-gray-400 block">Height</span>
                  <span className="font-mono text-lg">
                    {panelObject.data.height}
                  </span>
                </div>
                <div>
                  <span className="text-gray-400 block">Hash</span>
                  <span className="font-mono text-xs break-all text-gray-300">
                    {panelObject.data.hash}
                  </span>
                </div>
                <div>
                  <span className="text-gray-400 block">Time</span>
                  <span>
                    {new Date(panelObject.data.time).toLocaleString()}
                  </span>
                </div>
                <div>
                  <span className="text-gray-400 block">Transactions</span>
                  <span className="text-[#e8327e] font-bold">
                    {panelObject.data.txCount}
                  </span>
                </div>
                <button
                  onClick={() =>
                    handleLink(
                      `${MANTRA_SCAN_URL}/block/${panelObject.data.height}`
                    )
                  }
              ...
              <>
                ...
                <div>
                  ...
                  <span className="font-mono">{panelObject.data.height}</span>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <span className="text-gray-400 block">Gas Used</span>
                    <span className="font-mono text-[#e8327e]">
                      {panelObject.data.gasUsed.toLocaleString()}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-400 block">Gas Wanted</span>
                    <span className="font-mono text-gray-300">
                      {panelObject.data.gasWanted.toLocaleString()}
                    </span>
                  </div>
                </div>
                <div>
                  ...
                  <span
                    className={`font-bold ${
                      panelObject.data.success
                        ? "text-green-400"
                        : "text-red-400"
                    }`}
                  >
                    {panelObject.data.success ? "SUCCESS" : "FAILED"}
                  </span>
                </div>
                <button
                  onClick={() =>
                    handleLink(`${MANTRA_SCAN_URL}/tx/${panelObject.data.hash}`)
                  }
                  className="w-full mt-4 py-2 bg-blue-600 hover:bg-blue-500 rounded flex items-center justify-center gap-2 transition-colors"
                >
                  View on MantraScan <ExternalLink size={14} />
            ...
            <div className="text-gray-400 text-sm">Awaiting block data...</div>
          )}
        </div>
      </div>
    </div>
